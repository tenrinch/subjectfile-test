<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('user.department_id') ? 'invalid' : '' }}">
        <label class="form-label required" for="department">{{ trans('cruds.user.fields.department') }}</label>
        <x-select-list class="form-control" required id="department" name="department" :options="$this->listsForFields['department']" wire:model="user.department_id" />
        <div class="validation-message">
            {{ $errors->first('user.department_id') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.user.fields.department_helper') }}
        </div>
    </div>
